...
unsigned int BUFFER_WIDTH = 800;
unsigned int BUFFER_HEIGHT = 600;
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, gBuffer);
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
            glm::mat4 projection = glm::perspective(glm::radians(camera.Zoom), (float)BUFFER_WIDTH / (float)BUFFER_HEIGHT, 0.1f, 50.0f);
