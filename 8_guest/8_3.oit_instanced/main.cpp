...
int main(int argc, char* argv[])
{
	...
	// load models
    // -----------
    Model planet(RESOURCES_DIR"/objects/planet/planet.obj");
    ...
    for (unsigned int i = 0; i < amount; i++)
    {
        ...
        // 2. scale: Scale between 0.5 and 1.1f
        float scale = static_cast<float>((rand() % 60) / 100.0 + 0.5);
	...
	// ------------------------------------------------------------------
	glm::mat4 planetModelMat = calculate_model_matrix(glm::vec3(0.0f, -3.0f, 0.0f), glm::vec3(0.0f), glm::vec3(4.0f, 4.0f, 4.0f));
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// draw windows instances
		glBindVertexArray(transparentVAO);
        glBindTexture(GL_TEXTURE_2D, transparentTexture);
		transparentTexInstanceShader.use();
		transparentTexInstanceShader.setMat4("pv", vp);
