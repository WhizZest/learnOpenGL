...
int main(int argc, char* argv[])
{
	...
	// -------------------------
	Shader solidShader(VERTEX_SOLID_FILE, FRAGMENT_SOLID_FILE);
	Shader solidTexShader(VERTEX_SOLID_TEX_FILE, FRAGMENT_SOLID_TEX_FILE);
	Shader transparentShader(VERTEX_TRANSPARENT_FILE, FRAGMENT_TRANSPARENT_FILE);
	...
	glm::mat4 cubeModelMat1 = calculate_model_matrix(glm::vec3(-1.0f, 0.0f, -1.0f));
	glm::mat4 cubeModelMat2 = calculate_model_matrix(glm::vec3(2.0f, 0.0f, 0.0f));
	...
	std::vector<glm::mat4> windowsModelMats;
	for (auto& window : windows)
		windowsModelMats.push_back(calculate_model_matrix(window));
	
	solidTexShader.use();
	solidTexShader.setInt("texture1", 0);
    ...
    unsigned int transparentTexture = loadTexture(RESOURCES_DIR"/textures/window.png");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// cubes
		solidTexShader.use();
		solidTexShader.setMat4("mvp", vp * cubeModelMat1);
		glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, cubeTexture);
		glBindVertexArray(cubeVAO);
		glDrawArrays(GL_TRIANGLES, 0, 36);
		solidTexShader.setMat4("mvp", vp * cubeModelMat2);
		glDrawArrays(GL_TRIANGLES, 0, 36);
		// floor
        glBindVertexArray(planeVAO);
        glBindTexture(GL_TEXTURE_2D, floorTexture);
        solidTexShader.setMat4("mvp", vp * glm::mat4(1.0f));
        glDrawArrays(GL_TRIANGLES, 0, 6);
		...
		for (int i = 0; i < windowsModelMats.size(); i++)
		{
			transparentTexShader.setMat4("mvp", vp * windowsModelMats[i]);
			glDrawArrays(GL_TRIANGLES, 0, 6);
		}
